...
# This is an example script that creates a two-shard vttablet deployment. A
# replica and a master in each shard (so total of 4 vttablets)
...
# Start 2 vttablets for shard 0
for uid_index in 0 1; do
  uid=$[$uid_base1 + $uid_index]
  port=$[$port_base1 + $uid_index]
  mysql_port=$[$mysql_port_base1 + $uid_index]
...
# Start 2 vttablets for shard 1
for uid_index in 0 1; do
  uid=$[$uid_base2 + $uid_index]
  port=$[$port_base2 + $uid_index]
  mysql_port=$[$mysql_port_base2 + $uid_index]
  printf -v alias '%s-%010d' $cell $uid
  printf -v tablet_dir 'vt_%010d' $uid
